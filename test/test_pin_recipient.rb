...
class TestPinRecipient < MiniTest::Unit::TestCase
  ...
  def test_create_with_blank_email
    FakeWeb.register_uri(:post, 'https://test-api.pinpayments.com/1/recipients', body: fixtures['responses']['recipient']['blank_email'])
  ...
  def test_direct_update
    recipient = created_recipient
    FakeWeb.register_uri(:put, "https://test-api.pinpayments.com/1/recipients/#{recipient.token}", body: fixtures['responses']['recipient']['updated'])
  ...
  def test_find_recipient
    recipient = created_recipient
    FakeWeb.register_uri(:get, "https://test-api.pinpayments.com/1/recipients/#{recipient.token}", body: fixtures['responses']['recipient']['created'])
    recipient = PinPayment::Recipient.find(recipient.token)
    assert_kind_of PinPayment::Recipient, recipient
  end

  def test_fetch_all_recipients
    FakeWeb.register_uri(:get, 'https://test-api.pinpayments.com/1/recipients', body: fixtures['responses']['recipient']['all'])
  ...
  def test_create_recipient_with_bank_account_hash
    FakeWeb.register_uri(:post, 'https://test-api.pinpayments.com/1/recipients', body: fixtures['responses']['recipient']['create_with_bank_account'])
    data = {
