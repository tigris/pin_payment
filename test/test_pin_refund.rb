...
class TestPinRefund < MiniTest::Unit::TestCase
  ...
  def test_duplicate_refund
    FakeWeb.register_uri(:post, "https://test-api.pinpayments.com/1/charges/#{@charge.token}/refunds", body: fixtures['responses']['refund']['success'])
    PinPayment::Refund.create(@charge)
    FakeWeb.register_uri(:post, "https://test-api.pinpayments.com/1/charges/#{@charge.token}/refunds", body: fixtures['responses']['refund']['duplicate'])
