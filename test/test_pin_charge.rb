...
class TestPinCharge < MiniTest::Unit::TestCase
  ...
  def test_invalid_amount
    FakeWeb.register_uri(:post, 'https://test-api.pinpayments.com/1/charges', body: fixtures['responses']['charge']['invalid_amount'])
  ...
  def test_create_charge_with_card_hash
    FakeWeb.register_uri(:post, 'https://test-api.pinpayments.com/1/charges', body: fixtures['responses']['charge']['create_with_card'])
    charge = PinPayment::Charge.create(charge_hash)
    assert_equal true, charge.success?
  ...
  def test_create_charge_with_customer_token
    ...
    FakeWeb.register_uri(:post, 'https://test-api.pinpayments.com/1/charges', body: fixtures['responses']['charge']['create_with_customer'])
  ...
  def test_find_charge
    ...
    FakeWeb.register_uri(:get, "https://test-api.pinpayments.com/1/charges/#{charge.token}", body: fixtures['responses']['charge']['success'])
    charge = PinPayment::Charge.find(charge.token)
    assert_kind_of PinPayment::Charge, charge
  end

  def test_fetch_all_charges
    FakeWeb.register_uri(:get, 'https://test-api.pinpayments.com/1/charges', body: fixtures['responses']['charge']['all'])
